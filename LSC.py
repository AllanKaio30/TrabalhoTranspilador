...
def p_statement_assign(p):
    ...
    var_name = p[1]
    code_line = f"{var_name} = {p[3]['code']};"

    if var_name not in symbol_table:
        # Declarar a variável na primeira atribuição
        declaration = f"int {var_name} = {p[3]['code']};"
        symbol_table[var_name] = p[3]['value']
        if declaration not in c_code:
            c_code.append(declaration)
    else:
        # Apenas atribuir se o valor for diferente do último
        if symbol_table[var_name] != p[3]['value']:
            symbol_table[var_name] = p[3]['value']
            if code_line not in c_code:
                c_code.append(code_line)
...
def p_if_else_statement(p):
    ...
    if len(p) == 8:  # Apenas `if`
        ...
        for line in p[6]:
            # Preservar atribuições no bloco condicional
            c_code.append(line)
        c_code.append("}")
    elif len(p) == 12:  # `if-else`
        c_code.append(f"if ({p[3]['code']}) {{")
        for line in p[6]:  # Bloco `if`
            c_code.append(line)
        c_code.append("} else {")
        for line in p[10]:  # Bloco `else`
            c_code.append(line)
        c_code.append("}")
